        ...
        <EventErrorBoundary>
            <div className="p-4 sm:p-6">
                <div className="max-w-7xl mx-auto">
                    ...
                    <EventsFilters search={search} setSearch={setSearch} clubFilter={clubFilter} setClubFilter={setClubFilter} categoryFilter={categoryFilter} setCategoryFilter={setCategoryFilter} clubs={uniqueClubs} />
                    <EventsList events={filteredEvents} onEventClick={setSelectedEvent} />
                    {showCreateModal && <CreateEventModal open={showCreateModal} onClose={() => setShowCreateModal(false)} onSuccess={refetch} clubs={clubs} />}
                    {selectedEvent && <EventDetailsModal event={selectedEvent} open={!!selectedEvent} onClose={() => setSelectedEvent(null)} onUpdate={refetch} />}
                </div>
            </div>
        </EventErrorBoundary>
    );
